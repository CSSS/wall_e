...
class Administration(commands.Cog):
    ...
    async def load(self, ctx, name):
        ...
        if ctx.message.author in discord.utils.get(ctx.guild.roles, name="Bot_manager").members:
            ...
            logger.info("[Administration load()] unauthorized command attempt detected from "
    ...
    async def exc(self, ctx, *args):
        ...
        if ctx.message.author in discord.utils.get(ctx.guild.roles, name="Bot_manager").members:
            ...
            logger.info("[Administration exc()] unauthorized command attempt detected from "
                        + str(ctx.message.author))
            await ctx.send("You do not have adequate permission to execute this command, incident will be reported")
    ...
    def determineXYFrequency(self, dbConn, filters=None):
        ...
        if conn is None:
            return None
        dbCurr = conn.cursor()
        logger.info("[Administration determineXYFrequency()] trying to create a dictionary from " + str(dbCurr)
                    + " with the filters:\n\t" + str(filters))
        ...
        while len(results) > 0:
            logger.info("[Administration determineXYFrequency()] " + str(index) + "th index results of sql query=["
                        + str(results[0]) + "]")
            ...
            logger.info("[Administration determineXYFrequency()] where clause for determining which entries match "
                        "the entry [" + entry + "]:\n\t" + str(whereClause))
            sqlQuery = "select " + combinedFilter + " from commandstats WHERE " + whereClause + ";"
            logger.info("[Administration determineXYFrequency()] query that includes the above specified where "
                        "clause for determining how many elements match the filter of [" + entry + "]:\n\t"
                        + str(sqlQuery))
            dbCurr.execute(sqlQuery)
            resultsOfQueryForSpecificEntry = dbCurr.fetchall()
            frequency[entry] = len(resultsOfQueryForSpecificEntry)
            logger.info("[Administration determineXYFrequency()] determined that " + str(frequency[entry])
                        + " entries exist for filter " + entry)
            if index > 0:
            ...
            sqlQuery = """select """ + combinedFilter + """ from commandstats WHERE (
             """ + overarchingWHereClause + """ );"""
