...
async def send(ctx, content=None, tts=False, embed=None, file=None, files=None, delete_after=None, nonce=None, prefix=None, suffix=None):
	...
	try:
		...
		first = True # this is only necessary because it wouldnt make sense to have any potential embeds or file[s] with each message
		firstIndex = 0
		finished = False
		while not finished:
			if first:
				...
				logger.info("[send.py send()] messaage sent off with firstIndex = ["+str(firstIndex)+"] and lastIndex = ["+str(lastIndex)+"]")
				await ctx.send(formattedContent, tts=tts,embed=embed,file=file,files=files,delete_after=delete_after, nonce=nonce)
			else:
				formattedContent = content[firstIndex:lastIndex]
				if prefix is not None:
					formattedContent = prefix + formattedContent
